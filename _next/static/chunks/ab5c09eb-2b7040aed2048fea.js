 ...
 */ let{getBroadcastDims:a}=r.backend_util;function l(e,t,n){let l,C;let T=[];if(e.forEach(e=>{let t=r.util.sizeFromShape(e.shapeInfo.logicalShape);if(e.shapeInfo.isUniform?T.push(`uniform float ${e.name}${t>1?`[${t}]`:""};`):(T.push(`uniform sampler2D ${e.name};`),T.push(`uniform int offset${e.name};`)),n.enableShapeUniforms){let{uniformShape:i}=$(n.packedInputs,e.shapeInfo.logicalShape,e.shapeInfo.texShape);switch(i.length){case 1:T.push(`uniform int ${e.name}Shape;`);break;case 2:T.push(`uniform ivec2 ${e.name}Shape;`);break;case 3:T.push(`uniform ivec3 ${e.name}Shape;`);break;case 4:T.push(`uniform ivec4 ${e.name}Shape;`)}T.push(`uniform ivec2 ${e.name}TexShape;`)}}),n.enableShapeUniforms){switch(t.logicalShape.length){case 1:T.push("uniform int outShape;");break;case 2:T.push("uniform ivec2 outShape;"),T.push("uniform int outShapeStrides;");break;case 3:T.push("uniform ivec3 outShape;"),T.push("uniform ivec2 outShapeStrides;");break;case 4:T.push("uniform ivec4 outShape;"),T.push("uniform ivec3 outShapeStrides;")}T.push("uniform ivec2 outTexShape;")}n.customUniforms&&n.customUniforms.forEach(e=>{T.push(`uniform ${e.type} ${e.name}${e.arrayIndex?`[${e.arrayIndex}]`:""};`)});let m=T.join("\n"),R=e.map(e=>(function(e,t,n=!1,o){let l="";n?l+=function e(t,n){let o=t.shapeInfo.logicalShape;switch(o.length){case 0:return function(e){let t=e.name,n="get"+t.charAt(0).toUpperCase()+t.slice(1),r=(0,i.A)();return`
  ...
  `}(t,n);case 3:return function(t,n){let r=t.shapeInfo.logicalShape,o=t.name,a="get"+o.charAt(0).toUpperCase()+o.slice(1),l=t.shapeInfo.texShape,u=[Math.ceil(l[0]/2),Math.ceil(l[1]/2)];if(1===r[0]){let c=r.slice(1),p=f(t,c);return`
        ${e(p,n)}
  ...
  `;let h=u[0],s=u[1],d=Math.ceil(r[2]/2),v=d*Math.ceil(r[1]/2);return`
    vec4 ${a}(int b, int row, int col) {
      vec2 uv = packedUVfrom3D(
        ${h}, ${s}, ${v}, ${d}, b, row, col);
      return ${x.texture2D}(${o}, uv);
  ...
  `;let a=e.shapeInfo.logicalShape,l=a.length,u=e.shapeInfo.texShape,c=[Math.ceil(u[0]/2),Math.ceil(u[1]/2)],p=c[0],x=c[1],h=Math.ceil(a[l-1]/2),s=h*Math.ceil(a[l-2]/2),d="int b, int row, int col",v=`b * ${s} + (row / 2) * ${h} + (col / 2)`;for(let $=2;$<l-1;$++)d=`int b${$}, `+d,s*=a[l-$-1],v=`b${$} * ${s} + `+v;return`
    vec4 ${r}(${d}) {
      int index = ${v};
      int texR = index / ${x};
  ...
  `}(t,n);case 2:return function(t,n){let i=t.shapeInfo.logicalShape,o=t.name,a="get"+o.charAt(0).toUpperCase()+o.slice(1),l=t.shapeInfo.texShape;if(null!=l&&r.util.arraysEqual(i,l)){if(n)return`
    ...
    `;let u=l[0],c=l[1];return`
    float ${a}(int row, int col) {
      vec2 uv = (vec2(col, row) + halfCR) / vec2(${c}.0, ${u}.0);
      return sampleTexture(${o}, uv);
    }
  `}let{newShape:p,keptDims:x}=r.util.squeezeShape(i);if(p.length<i.length){let h=f(t,p);return`
      ${e(h,n)}
      float ${a}(int row, int col) {
...
`}(t,n);case 3:return function(t,n){let i=t.shapeInfo.logicalShape,o=t.name,a="get"+o.charAt(0).toUpperCase()+o.slice(1),l=i[1]*i[2],u=i[2],{newShape:c,keptDims:p}=r.util.squeezeShape(i);if(c.length<i.length){let x=f(t,c);return`
        ${e(x,n)}
  ...
  `;let T=s(o);return n?`
    ...
    `:`
      float ${a}(int row, int col, int depth) {
        // Explicitly use integer operations as dot() only works on floats.
        int index = row * ${l} + col * ${u} + depth + ${T};
        vec2 uv = uvFromFlat(${v}, ${$}, index);
        return sampleTexture(${o}, uv);
      }
  `}(t,n);case 4:return function(t,n){let i=t.shapeInfo.logicalShape,o=t.name,a="get"+o.charAt(0).toUpperCase()+o.slice(1),l=i[3],u=i[2]*l,c=i[1]*u,{newShape:p,keptDims:x}=r.util.squeezeShape(i);if(p.length<i.length){let h=f(t,p);return`
      ${e(h,n)}
    ...
    `;let v=t.shapeInfo.flatOffset,$=t.shapeInfo.texShape,C=$[0],T=$[1],m=`int stride2 = ${o}Shape[3];`,R=`int stride1 = ${o}Shape[2] * stride2;`,g=`int stride0 = ${o}Shape[1] * stride1;`;if(T===c&&null==v)return n?`
      float ${a}(int row, int col, int depth, int depth2) {
        ${m}
        ${R}
        float texR = float(row);
  ...
  `}(t,n);case 5:return function(t){let n=t.shapeInfo.logicalShape,i=t.name,o="get"+i.charAt(0).toUpperCase()+i.slice(1),a=n[4],l=n[3]*a,u=n[2]*l,c=n[1]*u,{newShape:p,keptDims:x}=r.util.squeezeShape(n);if(p.length<n.length){let h=f(t,p);return`
      ${e(h)}
    ...
    `;let v=t.shapeInfo.flatOffset,$=t.shapeInfo.texShape,C=$[0],T=$[1];if(T===c&&null==v)return`
      float ${o}(int row, int col, int depth, int depth2, int depth3) {
        int texR = row;
  ...
  `}(t);case 6:return function(t){let n=t.shapeInfo.logicalShape,i=t.name,o="get"+i.charAt(0).toUpperCase()+i.slice(1),{newShape:a,keptDims:l}=r.util.squeezeShape(n);if(a.length<n.length){let u=f(t,a);return`
      ${e(u)}
      float ${o}(int row, int col, int depth,
                    int depth2, int depth3, int depth4) {
        return ${o}(${S(["row","col","depth","depth2","depth3","depth4"],l)});
      }
    `}let c=n[5],p=n[4]*c,x=n[3]*p,h=n[2]*x,v=n[1]*h;if(t.shapeInfo.isUniform)return`
      float ${o}(int row, int col, int depth,
                  int depth2, int depth3, int depth4) {
        int index = round(dot(
          vec4(row, col, depth, depth2),
    ...
    float ${o}(int row, int col, int depth,
      ...
      int index = row * ${v} + col * ${h} + depth * ${x} +
          depth2 * ${p} + depth3 * ${c} + depth4 + ${R};
      vec2 uv = uvFromFlat(${T}, ${m}, index);
      return sampleTexture(${i}, uv);
    }
  `}(t);default:throw Error(`${i.length}-D input sampling is not yet supported`)}}(e,o);let u=e.shapeInfo.logicalShape,c=t.logicalShape;return u.length<=c.length&&(n?l+=function(e,t){let n;let i=e.name,o=i.charAt(0).toUpperCase()+i.slice(1),l=e.shapeInfo.logicalShape.length,u=t.logicalShape.length,c=a(e.shapeInfo.logicalShape,t.logicalShape),p=v(u),x=u-l,h=["x","y","z","w","u","v"];n=0===l?"":u<2&&c.length>=1?"coords = 0;":c.map(e=>`coords.${h[e+x]} = 0;`).join("\n");let s="";s=u<2&&l>0?"coords":e.shapeInfo.logicalShape.map((e,t)=>`coords.${h[t+x]}`).join(", ");let d="return outputValue;",$=r.util.sizeFromShape(e.shapeInfo.logicalShape),f=1===$,S=r.util.sizeFromShape(t.logicalShape),C=1===S;if(1!==l||f||C){if(f&&!C)d=1===u?`
        return vec4(outputValue.x, outputValue.x, 0., 0.);
      `:`
        return vec4(outputValue.x);
      `;else if(c.length){let T=l-2,m=l-1;c.indexOf(T)>-1&&c.indexOf(m)>-1?d="return vec4(outputValue.x);":c.indexOf(T)>-1?d="return vec4(outputValue.x, outputValue.y, outputValue.x, outputValue.y);":c.indexOf(m)>-1&&(d="return vec4(outputValue.xx, outputValue.zz);")}}else d=`
  ...
  `;let r=[Math.ceil(t[0]/2),Math.ceil(t[1]/2)],i=Math.ceil(e[e.length-1]/2),o=i*Math.ceil(e[e.length-2]/2),a=o,l="",u="b, r, c";for(let c=2;c<e.length-1;c++)a*=e[e.length-c-1],l=`
      int b${c} = index / ${a};
      index -= b${c} * ${a};
    `+l,u=`b${c}, `+u;return`
...
`;function h(){return`
  ...
  `}function v(e){if(e<=1)return"int";if(2===e)return"ivec2";if(3===e)return"ivec3";if(4===e)return"ivec4";if(5===e)return"ivec5";if(6===e)return"ivec6";throw Error(`GPU for rank ${e} is not yet supported`)}function $(e,t,n){let{newShape:i,keptDims:o}=r.util.squeezeShape(t),a=t.length,l=e&&3===a&&1===t[0],u=l?t.slice(1):i,c=!e&&a>1&&!r.util.arraysEqual(t,n)&&i.length<a||l;return{useSqueezeShape:c,uniformShape:c?u:t,keptDims:o}}function f(e,t){let n=JSON.parse(JSON.stringify(e));return n.shapeInfo.logicalShape=t,n}function S(e,t){return t.map(t=>e[t]).join(", ")}}}]);
//# sourceMappingURL=ab5c09eb-2b7040aed2048fea.js.map